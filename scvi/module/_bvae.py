...
class BVAE(BaseMinifiedModeModuleClass):
    ...
    def __init__(
        ...
        #THIS IS NEW CODE FOR THIS IMPLEMENTATION
        self.b_encoder(
            0, #no normal inputs, only the categorical inputs
            n_latent_b,
            n_cat_list=b_encoder_cat_list, #The batch parameter is considered a categorical variable at this stage, but switches to a latent at the decoder
            n_layers=n_layers,
            n_hidden=n_hidden,
            dropout_rate=dropout_rate,
            distribution=latent_distribution,
        ...
        #THIS IS ALSO MODIFIED BECAUSE THERE ARE NEW LATENTS THAT HAVE TO BE CONSIDERED FROM THE BATCH ENCODER
        n_latent = n_latent + n_latent_b
    ...
    def _regular_inference(
        ...
        qz, z = self.z_encoder(encoder_input, (), *categorical_input) ### MODIFIED THIS LINE AS WELL, VERY DIFFICULT


        zb, b = self.b_encoder((), batch_index, *categorical_input) ### COULD EASILY BE AN ERROR HERE, TEST IT OUT
    ...
    def generative(
        ...
        decoder_input = torch.cat([z, b], dim=-1)
    ...
    def loss(
        ...
        kl_divergence_b = kl(inference_outputs["qb"], generative_outputs["pb"]).sum(
            dim=-1
        )

        reconst_loss = -generative_outputs["px"].log_prob(x).sum(-1)

        kl_local_for_warmup = kl_divergence_z + kl_divergence_b #MODIFY THIS POTENTIALLY, MAYBE KL WARMUP WON'T HELP BATCH EFFECTS
        ...
        kl_local = {
            ...
            "kl_divergence_b": kl_divergence_b,
        }
        return LossOutput(
