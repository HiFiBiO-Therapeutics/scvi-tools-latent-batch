...
class JaxVAE(JaxBaseModuleClass):
    ...
    n_input: int  ##SORT OUT WHAT N_INPUT SHOULD BE
    n_batch: int
    n_hidden: Tunable[int] = 128
    n_latent: Tunable[int] = 30
    n_batch_latent: Tunable[int] = 5
    ...
    def setup(self):
        ...
        self.encoder = FlaxEncoder(
            n_input=self.n_input, ###Modify this
            n_latent=self.n_latent,
            n_hidden=self.n_hidden,
            dropout_rate=self.dropout_rate,
        )

        self.batch_encoder = FlaxEncoder(
            n_input=self.n_input, ###Modify this
            n_latent=self.n_batch_latent,
            n_hidden=self.n_hidden,
            dropout_rate=self.dropout_rate,
        )
    ...
    def inference(self, x: jnp.ndarray, n_samples: int = 1) -> dict:
        ...
        # batch encoder, TODO figure out how to subset only to samples in batch instead of all of x
        bmean, bvar = self.batch_encoder(x, training=self.training)
        bstddev = jnp.sqrt(bvar) + self.eps

        bqz = dist.Normal(bmean, bstddev)
        bz_rng = self.make_rng("z")
        bsample_shape = () if n_samples == 1 else (n_samples,)
        bz = qz.rsample(bz_rng, sample_shape=sample_shape)

        return {"qz": qz, "z": z, "bqz": bqz, "bz": bz}
    ...
    def generative(self, x, z, bz) -> dict:
        """Run generative model."""
        # one hot adds an extra dimension
        # batch = jax.nn.one_hot(batch_index, self.n_batch).squeeze(-2)
        # joint_z = jnp.concatenate((z, bz), axis=1)

        rho_unnorm, disp = self.decoder(z, bz, training=self.training)
        disp_ = jnp.exp(disp)
    ...
    def loss(
        ...
        bqz = inference_outputs["bqz"] #Added
        reconst_loss = -px.log_prob(x).sum(-1)
        kl_divergence_z = dist.kl_divergence(qz, dist.Normal(0, 1)).sum(-1)

        kl_divergence_bz = dist.kl_divergence(bqz, dist.Normal(0, 1)).sum(-1) #Added

        # kl_local_for_warmup = kl_divergence_z
        kl_local_for_warmup = kl_divergence_z + kl_divergence_bz #Added
